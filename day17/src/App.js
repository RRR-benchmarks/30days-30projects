...
const App = memo(function App() {
  ...
  const lastElementCallback = useCallback(node => {
    ...
    lastElement.current = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting) {
        setLoading(true);
        pageNumber.current = (prevPageNumber => prevPageNumber + 1)(pageNumber.current);
      }
    });
  ...
  useEffect(() => {
    if (loading) {
      setLoading(false);
      setShow(prevShow => prevShow.concat(Array.from({
        length: 10
      }, (_, index) => 10 * (pageNumber.current - 1) + index + 1)));
    }
  }, [loading, pageNumber.current]);
