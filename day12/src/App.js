...
const App = memo(function App() {
  ...
  const handleChange = e => {
    if (e.target.value.length < 3 && (/^\d+$/.test(e.target.value) || e.target.value === '')) {
      length.current.value = e.target.value;
    }
  };
  const handleClick = () => {
    let indexCheck = 0;
    for (indexCheck; indexCheck <= 3; indexCheck++) {
      if (checkbox.current[indexCheck] === true) break;
    }
    if (indexCheck === 4) {
      alert('Please check at least 1 box');
      return;
    }
    const charsets = ["!@#$%^&*()_", "0123456789", "abcdefghijklmnopqrstuvwxyz", "ABCDEFGHIJKLMNOPQRSTUVWXYZ"];
    let pw = '';
  ...
  function handleCheck(e, i) {
    const newCheckbox = [...checkbox.current];
    newCheckbox[i] = e.target.checked;
    checkbox.current = newCheckbox;
  }
  return <>
      <div className="container">
        ...
        <div className="box-checker password">
          <p>Password Length: {length.current.value}</p>
          <input ref={length} type="range" min="4" max="32"></input>
        </div>
